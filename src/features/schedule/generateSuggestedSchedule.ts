...
function listWeekdaysAfterSunday(
  dateISO: DateISO,
  daysSet: Set<DateISO>,
): DateISO[] {
...
export function generateSuggestedSchedule(input: Input): ScheduleAssignments {
  ...
  const fixedSundayTalesRule = isEnabledRule(
    input.rules,
    "fixed_off_sunday_tales",
  );

  const cookRoleId = asString(cookRotationRule?.params.cookRoleId);
  const exactlyOffCount =
    asNumber(cookRotationRule?.params.exactlyOffCount) ?? 1;
