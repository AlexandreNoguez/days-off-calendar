...
function statusOf(
  ...
  const status = assignments[employeeId]?.[dateISO] ?? "WORK";
  return status === "OFF" ? "Folga" : "Trabalho";
}
...
export function buildWorkbook(input: Input): Uint8Array {
  ...
  const rows: Array<Array<string | number>> = [
    ["Escala de Folgas", `${String(input.month).padStart(2, "0")}/${input.year}`],
    [
      "Status da validação",
      input.validation.isValid ? "Sem conflitos HARD" : "Com conflitos HARD",
    ],
    ["Conflitos HARD", input.validation.conflicts.filter((c) => c.severity === "HARD").length],
    ["Conflitos SOFT", input.validation.conflicts.filter((c) => c.severity === "SOFT").length],
    [],
    header,
  ];

  input.employees.forEach((employee) => {
    const statuses = input.daysOfMonth.map((d) => statusOf(input.assignments, employee.id, d));
    const offCount = statuses.reduce((acc, status) => acc + (status === "Folga" ? 1 : 0), 0);
